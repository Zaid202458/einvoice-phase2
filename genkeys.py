...
def generatekeys():
    ...
    subprocess.run([r"C:\OpenSSL-Win64\bin\openssl.exe", 'ecparam',
                    '-name', 'secp256k1', '-genkey', '-noout', '-out', 'PrivateKey.pem'])

    # Generate public key from private key
    subprocess.run([r"C:\OpenSSL-Win64\bin\openssl.exe", 'ec',
                    '-in', 'PrivateKey.pem', '-pubout', '-out', 'publickey.pem'])

    # Generate CSR
    subprocess.run([r"C:\OpenSSL-Win64\bin\openssl.exe", 'req', '-new', '-sha256', '-key', 'PrivateKey.pem', '-extensions', 'v3_req',
